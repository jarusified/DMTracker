...
#define NVPW_CALL(call)                                                        \
  [&]() -> bool {                                                              \
    ...
    if (_status_ != NVPA_STATUS_SUCCESS) {                                     \
      LOG(WARNING) << fmt::format("function {} failed with error ({})", #call, \
                                  (int)_status_);                              \
      return false;                                                            \
    }                                                                          \
    return true;                                                               \
...
CuptiProfilerResult evalMetricValues(
    const std::string &chipName, const std::vector<uint8_t> &counterDataImage,
    const std::vector<std::string> &metricNames, bool verbose = false);
