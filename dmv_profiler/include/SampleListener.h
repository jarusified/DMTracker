...
class SampleValue {
public:
  template <class T> explicit SampleValue(T v) { init<T>(v); }
...
template <> inline void SampleValue::init(uint64_t v) {
  int_ = v, type_ = INT64;
}
template <> inline void SampleValue::init(int64_t v) {
  int_ = v, type_ = INT64;
}
template <> inline void SampleValue::init(int v) { int_ = v, type_ = INT64; }
template <> inline void SampleValue::init(double v) {
  dbl_ = v, type_ = DOUBLE;
}
