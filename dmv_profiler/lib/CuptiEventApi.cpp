...
CuptiEventApi::createGroupSets(vector<CUpti_EventID> &ids) {
...
void CuptiEventApi::enablePerInstance(CUpti_EventGroup eventGroup) {
  ...
  CUPTI_CALL(cuptiEventGroupSetAttribute(
      eventGroup, CUPTI_EVENT_GROUP_ATTR_PROFILE_ALL_DOMAIN_INSTANCES,
...
void CuptiEventApi::readEvent(CUpti_EventGroup grp, CUpti_EventID id,
                              vector<int64_t> &vals) {
  size_t s = sizeof(int64_t) * vals.size();
  CUPTI_CALL(
      cuptiEventGroupReadEvent(grp, CUPTI_EVENT_READ_FLAG_NONE, id, &s,
...
vector<CUpti_EventID> CuptiEventApi::eventsInGroup(CUpti_EventGroup grp) {
  ...
  CUPTI_CALL(cuptiEventGroupGetAttribute(grp, CUPTI_EVENT_GROUP_ATTR_NUM_EVENTS,
                                         &s, &group_size));
  size_t events_size = group_size * sizeof(CUpti_EventID);
  vector<CUpti_EventID> res(group_size);
  CUPTI_CALL(cuptiEventGroupGetAttribute(grp, CUPTI_EVENT_GROUP_ATTR_EVENTS,
                                         &events_size, res.data()));
  return res;
}
