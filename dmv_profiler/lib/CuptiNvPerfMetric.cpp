...
bool getRawMetricRequests(
    NVPA_MetricsContext *metricsContext, std::vector<std::string> metricNames,
...
bool getProfilerConfigImage(const std::string &chipName,
  ...
  if (!getRawMetricRequests(metricsContextCreateParams.pMetricsContext,
                            metricNames, rawMetricDeps, rawMetricRequests)) {
    return false;
  }
...
#if CUDART_VERSION < 11040
  if (!NVPW_CALL(NVPA_RawMetricsConfig_Create(&metricsConfigOptions,
                                              &rawMetricsConfig))) {
    return false;
  }
#else
...
std::string getRangeDescription(const std::vector<uint8_t> &counterDataImage,
                                int rangeIndex) {
...
CuptiProfilerResult
evalMetricValues(const std::string &chipName,
                 const std::vector<uint8_t> &counterDataImage,
                 const std::vector<std::string> &metricNames, bool verbose) {
