...
void atomic_cb(CUpti_CallbackDomain /*domain*/, CUpti_CallbackId /*cbid*/,
               const CUpti_CallbackData * /*cbInfo)*/) {
...
void empty_cb(CUpti_CallbackDomain /*domain*/, CUpti_CallbackId /*cbid*/,
              const CUpti_CallbackData * /*cbInfo*/) {}
...
TEST(CuptiCallbackApiTest, SimpleTest) {
  ...
  auto addSimpleCallback = [&]() -> bool {
    bool ret =
        api.registerCallback(CUPTI_CB_DOMAIN_RUNTIME_API,
  ...
  bool ret =
      api.deleteCallback(CUPTI_CB_DOMAIN_RUNTIME_API,
                         CuptiCallbackApi::CUDA_LAUNCH_KERNEL, &simple_cb);

  EXPECT_TRUE(ret) << "Failed to remove callback";

  ret = api.deleteCallback(CUPTI_CB_DOMAIN_RUNTIME_API,
                           CuptiCallbackApi::CUDA_LAUNCH_KERNEL, &atomic_cb);
...
TEST(CuptiCallbackApiTest, AllCallbacks) {
  ...
  EXPECT_TRUE(testCallback(CUPTI_CB_DOMAIN_RESOURCE,
